...
@*/
	for( i = n-1; i > 0 ; i-- )
	{
		if ( n % i == 0 )
		{
			...
			int j;
/*
(j >= 2) && (isPrime(i) => is_prime == 1) && ((is_prime == 0) || (is_prime == 1)) && ((is_prime == 1) => (isPrime(i) || (\exists int k; (k >= j) && (k < i) && (isDivisor(k, i)))))

(\forall int k; (k >= 2) && (k < j) => (!isDivisor(k, i) || is_prime != 0)) &&
*/
/*@
@ invariant (j >= 2) && ((i >= 2) => (j <= i)) && ((i == 1) => (is_prime != 0)) &&
